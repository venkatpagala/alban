    ...
    implements UserManageableService
{
        ...
        throws UserReadManageableException
    {
        try
        {
            Query query = emanager.createQuery("select entity from User as entity");
            query.setMaxResults(250);
            ...
            CollectionUtils.transform(entities, new Transformer()
            {
                public Object transform(final Object object)
                {
                    ...
                    {
                        Collection<Boolean> rolesLabels = Collections.synchronizedCollection(new ArrayList<Boolean>());
                        for (final Iterator<UserRole> iterator = result.getRoles().iterator(); iterator.hasNext();)
                        {
                            UserRole entity = iterator.next();
                            synchronized (rolesLabels)
                            {
                                rolesLabels.add(entity.getConditional());
                            }
                        }
                        result.setRolesLabels(rolesLabels);
                    }
                    return result;
                }
            });
        ...
        throws UserReadManageableException
    {
        ...
        try
        {
            lists.put("roles", emanager.createQuery("select item.id, item.conditional from UserRole as item order by item.conditional").getResultList());
        }
        catch (Exception ex)
        {
            throw new UserReadManageableException(ex);

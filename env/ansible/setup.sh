...
if [ "$OPTARG" != "" ]; then
    ...
    sudo PYTHONUNBUFFERED=x ANSIBLE_FORCE_COLOR=$ANSIBLE_FORCE_COLOR ANSIBLE_ERROR_ON_UNDEFINED_VARS=True ansible-playbook -i hosts -c local -v -e "$OPTARG" site.yml | tee setup.log
else
    sudo PYTHONUNBUFFERED=x ANSIBLE_FORCE_COLOR=$ANSIBLE_FORCE_COLOR ANSIBLE_ERROR_ON_UNDEFINED_VARS=True ansible-playbook -i hosts -c local -v site.yml | tee setup.log
fi
RC=$?
if [ ${RC} -ne 0 ]; then

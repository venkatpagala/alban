...
set USER_PROFILE=${CONFIG_DIR}/profiles/${DEV_USER}.dev.properties
...
if ( ${?PROJECT_VERSION} ) then
  if ( "${DEV_USER}" == "${PROJECT_USER}${PROJECT_VERSION}" ) then
    set USER_PROFILE=${CONFIG_DIR}/profiles/${PROJECT_USER}.dev.properties
...
if ( ${GENERATION_MODE} != "user" ) then
  ...
  setenv | grep -v ! | grep -v \( | perl -e'while(<STDIN>){ (/[^=]*=.*/) || next;  if ( (/^(USER=|HOME=|ARCH=|MACHINE=|PROJECT_USER=|HOST=|DISPLAY=|PATH=|TERM=)/) ) { s/([^=]*)=(.*)/unset $1/ } else { s/([^=]*)=(.*)/unset $1\nunsetenv $1/ } print $_; }'> ${UNSET_FILE}
...
if ( "${QUIET_MODE}" != "Yes" ) then 
  echo "SOURCE extraction: 	${PROJECT_DEV}"

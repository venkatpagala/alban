...
public class BasicPaymentMethod implements IPaymentMethod
{
    ...
    public static BigDecimal calcMonthlyPayment(BigDecimal monthlyInterestRate, BigDecimal loanAmount, int numberOfMonths){
        ...
        return loanAmount.multiply(monthlyInterestRate).divide(new BigDecimal(1).subtract(
                (new BigDecimal(1).divide((monthlyInterestRate.add(new BigDecimal(1))), MathContext.DECIMAL128).pow(numberOfMonths)) ), MathContext.DECIMAL128);
    }

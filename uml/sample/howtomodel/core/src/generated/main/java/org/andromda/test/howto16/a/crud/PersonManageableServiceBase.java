    ...
    implements PersonManageableService
{
    ...
    {
        try{
            return toValueObject((Person)emanager.find(Person.class, serial));
        }
        catch (Exception ex)
        {
            throw new PersonReadManageableException(ex);
     ...
     * @throws PersonReadManageableException
     */
    public List<PersonValueObject> read(String name, Date birthDate, String serial, String[] cars)
        throws PersonReadManageableException
    {
     ...
     * @throws PersonReadManageableException
     */
    public List<PersonValueObject> readAll()
        throws PersonReadManageableException
    {
        try
        {
    ...
    private static List<PersonValueObject> toValueObjects(Collection<Person> entities)
    {
        final List<PersonValueObject> list = new ArrayList<PersonValueObject>();

        for (Iterator<Person> iterator = entities.iterator(); iterator.hasNext();)
        {
    ...
    private static PersonValueObject toValueObject(Person entity)
    {
        final PersonValueObject valueObject = new PersonValueObject();

        valueObject.setName(entity.getName());
        valueObject.setBirthDate(entity.getBirthDate());
        valueObject.setSerial(entity.getSerial());

        final Collection<Car> cars = entity.getCars();
        if (cars == null || cars.isEmpty())
        {
            valueObject.setCars(null);
        }
        else
        {
            final String[] values = new String[cars.size()];
            int counter = 0;
            for (final Iterator<Car> iterator = cars.iterator(); iterator.hasNext();counter++)
            {
                final Car element = iterator.next();
                values[counter] = element.getSerial();
            }
            valueObject.setCars(values);
        }
